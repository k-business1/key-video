...
(function requireLogin(){
  try{
    const u = localStorage.getItem("km_u");
...
function uploadToCloudinary(file, resourceType, barId, pctId){
  return new Promise((resolve, reject)=>{
    ...
    xhr.timeout = 120000; // 2 min — large videos on slow connections need real headroom
    ...
    xhr.onload = function(){
      if(xhr.status >= 200 && xhr.status < 300){
        try{
          ...
          reject(new Error("Cloudinary sent back something unexpected — check the cloud name / upload preset are correct"));
        }
      } else {
        // Cloudinary returns a JSON body even on failure — surface its real message
        // (e.g. "Upload preset not found", "Invalid image file") instead of just a status code.
    ...
    xhr.ontimeout = function(){
      reject(new Error("The upload timed out — your connection may be too slow for this file size, or the request never reached Cloudinary"));
    };
    xhr.onerror = function(){
      if(window.location.protocol === "file:"){
        reject(new Error("This page was opened as a local file — uploads only work when the site is loaded over https:// (your live site), not by opening the HTML file directly"));
      } else if(navigator.onLine === false){
        reject(new Error("No internet connection detected"));
      } else {
        reject(new Error("Upload blocked before reaching Cloudinary — if you're using a VPN, turn it off (or exclude this site) and try again. Could also be an ad-blocker, or this domain isn't allowed in your Cloudinary account's Security settings"));
      }
    };
    xhr.send(fd);
  });
