...
function bootApp(after){
  try{var u=localStorage.getItem('km_u');if(u)S.user=JSON.parse(u);}catch(e){}
  updateHeaderUI();
...
function doLogin(){
  ...
  api('login',{gmail:em,password:pw},function(r){
    if(r.ok){S.user=r.user;localStorage.setItem('km_u',JSON.stringify(r.user));cOv('authOv');updateHeaderUI();loadNotifs();startPing();toast('Welcome, '+r.user.name+' 👋','tok');if(window.onAuthChanged)onAuthChanged();}
...
function doRegister(){
  ...
  api('register',{name:nm,gmail:em,password:pw,country:co},function(r){
    if(r.ok){S.user=r.user;localStorage.setItem('km_u',JSON.stringify(r.user));cOv('authOv');updateHeaderUI();startPing();toast('Welcome '+nm+' 🎉','tok');if(window.onAuthChanged)onAuthChanged();}
    else er.textContent=r.msg;
  });
}
function logout(){S.user=null;localStorage.removeItem('km_u');clearInterval(S.pingTimer);updateHeaderUI();toast('Signed out 👋');if(window.onAuthChanged)onAuthChanged();}
